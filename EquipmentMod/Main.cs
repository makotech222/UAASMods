...
namespace EquipmentMod
{
    internal static class Main
    {
        ...
        private class CampaignController_Patch
        {
            ...
            {
                LoadLandEquipment(__result);
                LoadShipModules(__result);
                LoadNavalEquipment(__result);
            }

            private static void LoadShipModules(Game __result)
            {
            ...
            private static void LoadLandEquipment(Game __result)
            {
                var Initialized = true;
                string configFile = @"Mods/EquipmentMod/Rifles.txt";
                if (!File.Exists(configFile))
                {
                    File.CreateText(configFile).Close();
                    Initialized = false;
                ...
                if (!Initialized)
                {
                    var weapons = __result.campaign.campaignControllerData.campSettings.shopSettings.riflePool.rifles;
                    if (weapons == null)
                        return;
                    var slimModules = new List<WeaponHotpatch>();
                    foreach (var mod in weapons)
                    {
                        slimModules.Add(new WeaponHotpatch(mod));
                    }
                    File.WriteAllText(configFile, JsonConvert.SerializeObject(slimModules, Formatting.Indented, new JsonSerializerSettings() { ReferenceLoopHandling = ReferenceLoopHandling.Ignore, MaxDepth = 2 }));
                }
                else
                {
                    var updated = JsonConvert.DeserializeObject<List<WeaponHotpatch>>(File.ReadAllText(configFile));
                    var current = __result.campaign.campaignControllerData.campSettings.shopSettings.riflePool.rifles;
                    foreach (var updatedItem in updated)
                    {
                        try
                        {
                            var matching = current.Find(x => x.textName.ToString() == updatedItem.ID);
                            updatedItem.Update(ref matching);
                            matching.OnAfterDeserialize();
                        }
                        catch (Exception)
                        {
                        }
            ...
            private static void LoadNavalEquipment(Game __result)
            {
                var Initialized = true;
                string configFile = @"Mods/EquipmentMod/Cannons.txt";
                if (!File.Exists(configFile))
                {
                    File.CreateText(configFile).Close();
                    Initialized = false;
                ...
                if (!Initialized)
                {
                    var weapons = __result.campaign.campaignControllerData.campSettings.shopSettings.cannonPool.cannons;
                    if (weapons == null)
                        return;
                    var slimModules = new List<CannonHotpatch>();
                    foreach (var mod in weapons)
                    {
                        slimModules.Add(new CannonHotpatch(mod));
                    }
                    File.WriteAllText(configFile, JsonConvert.SerializeObject(slimModules, Formatting.Indented, new JsonSerializerSettings() { ReferenceLoopHandling = ReferenceLoopHandling.Ignore, MaxDepth = 2 }));
                }
                else
                {
                    var updated = JsonConvert.DeserializeObject<List<CannonHotpatch>>(File.ReadAllText(configFile));
                    var current = __result.campaign.campaignControllerData.campSettings.shopSettings.cannonPool.cannons;
                    foreach (var updatedItem in updated)
                    {
                        try
                        {
                            var matching = current.Find(x => x.title.ToString() == updatedItem.ID);
                            updatedItem.Update(ref matching);
                            matching.OnAfterDeserialize();
                        }
                        catch (Exception)
